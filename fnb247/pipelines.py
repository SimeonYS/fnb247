...
class Ffnb247Pipeline:

    # Database setup
    conn = sqlite3.connect('fnb247.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `fnb247`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM fnb247 WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO fnb247 (date, title, link, content)"
